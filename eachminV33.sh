...
touch /home/centos/scripts/Node/data/UAMAZCacum.txt
...
chown centos:centos /home/centos/scripts/Node/data/UAMAZCacum.txt
...
#                            mysql -A -u root -pyorgos84 test << EOF
                ...
                if [[ $checkuamazc == "SI" ]]
                    then
                echo  uamazc_$MinAterior >> $logdir/ArchivosMinuto.log
                elif [[ $checkuamazc == "NO" ]]
                    then
                echo  uamazc_$MinAterior >> $logdir/ArchivosFaltantes$fechaActual.log
                fi
        ...
        if [[ "$filtro" == "SI" ]]; then
                if [[ $file == *.oh  ]]; then
                    ...
                    gunzip -c $path$file > /tmp/file.txt 
                    while IFS= read -r line; do 
                        ...
                        if [[ $compara == "07:" ]]; then
                                reflectmp2=`echo $line | cut -d ":" -f 2`
                                #reflec=`calc $reflec + 0`
                                reflec2=`printf '%0.3f\n' "$reflectmp2"`
                                #reflec=`echo $reflec`
                                if [[ $reflec2 == 0.000 ]]; then
                                    reflec2=-9.999
                                fi
                        ...
                        if [[ $compara == "50:" ]]; then
                            ...
                            touch $dir2/$estacion$aniomes.csv
                            touch $dir2/$estacion.csv
                            echo  $estacion";"$fechaS2";"$horaS";"$inten2";"$acum";"$reflec2";"$visi2";"$gotas2";"$cine2 
                            echo  $estacion";"$fechaS2";"$horaS";"$inten2";"$acum";"$reflec2";"$visi2";"$gotas2";"$cine2 >> $dir2/$estacion$aniomes.csv
                            echo  $estacion$coma$fechap$coma$horaS$coma$inten2$coma$acum > $dir2/a$estacion.csv
                            ...
                            gzip -c $dir2/$estacion$aniomes.csv > $dir3/$estacion2/$estacion.gz
                            ...
                            echo "$estacion;$coord;$inten2;$acum" >> $dir/$datos2.txt
                            echo "$estacion,$inten2,$acum" > $dir/$estacion.txt
                            #echo "$actual,$estacion,$acum2,$inten2,$fecha" >> $dir/$estacion"acum.txt"
                            tiempoEstacion=$fechaEstacion" "$horaEstacion
                            echo $tiempoEstacion;
                            echo $estacion";"$tiempoServidor";"$tiempoEstacion";"$inten2";"$acum";"$acumuladamas";"$visi2";"$gotas2";"$voltaje2";"$reflec2";"$tipolluvia";"$cine2 >> $dir/Abases.csv
#                            mysql -A -u root -pyorgos84 test << EOF
#                            INSERT INTO estaciones (estacion,fecha_servidor,fecha_estacion,intensidad,acumulada,acumuladaplus,visibi,gotas,volta,reflec,tipo,cinetica) VALUES ('$estacion', '$tiempoServidor', '$tiempoEstacion', '$inten2', '$acum', '$acumuladamas', '$visi2', '$gotas2', '$voltaje2', '$reflec2', '$tipolluvia', '$cine2'); 
#EOF
                    ...
                    while IFS= read -r line; do
                        ...
                        if [[ $compara == "07:" ]]; then
                                reflectmp=`echo $line | cut -d ":" -f 2`
                                #reflec=`calc $reflec + 0`
                                reflec=`printf '%0.3f\n' "$reflectmp"`
                                #reflec=`echo $reflec`
                                if [[ $reflec == 0.000 ]]; then
                                    reflec=-9.999
                                fi
                        ...
                        if [[ $compara = "50:"  ]]; then
                            ...
                            touch $dir2/$estacion$aniomes.csv
                             touch $dir2/$estacion.csv
                            aver=`echo  $estacion$coma$fechaC$coma$horaS$coma$inten$coma$acum$coma$reflec`
                            aver2=`echo  $visi";"$gotas";"$cine`
                            echo $aver
                            echo $aver2
                            echo $aver";"$aver2
                            echo  $estacion$coma$fechaC$coma$horaS$coma$inten$coma$acum$coma$reflec$coma$visi$coma$gotas$coma$cine >> $dir2/$estacion$aniomes.csv
                            ...
                            tiempoEstacion=$fechaEstacion" "$horaEstacion
                            echo $tiempoEstacion;
                            echo $estacion";"$tiempoServidor";"$tiempoEstacion";"$inten";"$acum";"$acumuladamas";"$visi";"$gotas";"$voltaje";"$reflec";"$tipolluvia";"$cine >> $dir/Abases.csv
#                            mysql -A -u root -pyorgos84 test << EOF
